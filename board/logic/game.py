...
class GameAPI:
    def __init__(self, game_id):
        ...
        if "cover" in res:
            self.img_url = igdb_wrapper.get_img_url(
                res["cover"]["image_id"], size="cover_big"
            )
        else:
            self.img_url = igdb_wrapper.get_img_url(
                res["screenshots"][0]["image_id"], size="cover_big"
            )
        if "release_dates" in res:
            self.release = make_aware(
                datetime.datetime.fromtimestamp(res["release_dates"][0])
            )
        else:
            self.release = None
        self.screen_url = [
        ...
        if "aggregated_rating" in res:
            self.aggregated_rating = [
                res["aggregated_rating"],
                res["aggregated_rating_count"],
            ]
        else:
...
class Game:
    def __init__(self, game_id):
        ...
        self.screen_url = [
            item.url for item in game.images.filter(game=game_id, is_cover=False)
        ]
